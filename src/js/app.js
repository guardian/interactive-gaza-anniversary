            ...
            if (label.childrenLength > 50 || keyTo === "monthYear") {
                if (label.id.split("/").length !== 3) {
                    label.id.split(` (`).forEach((id, m) => {
                        ...
                        context.strokeText(id, Math.round(label.x * (circleScaler)) + xPadding, Math.round(label.y * (circleScaler) + (fontScale * 24) + yPadding + (fontScale*extraY)));

                        // context.globalAlpha = easedPlabels;
                        context.font = `400 ${14*(fontScale)}px Guardian Text Sans Web`;
                        context.fillStyle = "#000";
                        context.textAlign = "center";
                        context.fillText(id, Math.round(label.x * (circleScaler)) + xPadding, Math.round(label.y * (circleScaler) + (fontScale * 24) + yPadding + (fontScale *extraY)));
                    });
...
function labelEase(t) {
    return Math.min(t * t * t * 2, 1)
}
...
const renderCharts = (chartDataRaw) => {
    ...
    const charts = [{
        ...
        },
        {
            ...
            tickFormat: null,
            startAt: 340,
            bigNumber: `<div><span>$357</span> per capita as of Sep 2018</div>`,
    ...
    charts.forEach((chartObj, i) => {
        ...
        let chartData = (nestedData.find(d => d.key === chart)).values
            .map(d => Object.assign({}, d, {
                "formattedDate": new Date(d.date.split("/")[1] + "/" + d.date.split("/")[0] + "/20" + d.date.split("/")[2]),
                "numValue": Number(d.Value)
            }))
        ...
        if (chart === "Total number of medical applications submitted to EREZ Crossings") {
            ...
            chartData = chartData.map((d, i) => Object.assign({}, d, {
                "% of denied and delayed permits to cross EREZ": Number(permits[i].Value)
            }))
